...
resource "aws_instance" "main" {
  count         = length(data.tfe_output.networking.nonsensitive_values.public_subnets)
  ami           = nonsensitive(data.aws_ssm_parameter.amzn2_linux.value)
  instance_type = var.instance_type
  subnet_id     = data.tfe_output.networking.nonsensitive_values.public_subnets[count.index]
...
resource "aws_lb" "main" {
  ...
  subnets            = data.tfe_output.networking.nonsensitive_values.public_subnets
...
resource "aws_lb_target_group" "main" {
  ...
  protocol    = "HTTP"
  vpc_id      = var.data.tfe_output.networking.nonsensitive_values.vpc_id
}
